...
export class ActividadService {
  ...
  protected convertDateFromClient(actividad: IActividad): IActividad {
    return Object.assign({}, actividad, {
      fechaInicio: actividad.fechaInicio?.isValid() ? actividad.fechaInicio.toJSON() : undefined,
      fechaFin: actividad.fechaFin?.isValid() ? actividad.fechaFin.toJSON() : undefined,
    });
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((actividad: IActividad) => {
        actividad.fechaInicio = actividad.fechaInicio ? dayjs(actividad.fechaInicio) : undefined;
        actividad.fechaFin = actividad.fechaFin ? dayjs(actividad.fechaFin) : undefined;
      });
    }
    return res;
  }
